...
export type IssueCategory = 'contrast' | 'font-size' | 'alt-text' | 'reading-order' | 'color-only' | 'motion' | 'language' | 'structure';
...
export class AccessibilityService {
  ...
  async checkProject(
    ...
    const themeColors = (project.theme?.colors as any) || {};
  ...
  async suggestAccessibleColors(
    ...
    let adjustedFg = this.adjustColorForContrast(foreground, background, targetRatio);
    ...
    let adjustedBg = this.adjustColorForContrast(background, foreground, targetRatio);
  ...
  async autoFixIssues(
    ...
    const issues = (report.issues as unknown as AccessibilityIssue[])
      .filter(i => issueIds.includes(i.id) && i.autoFixable);
    ...
    for (const issue of issues) {
      try {
        ...
        fixed++;
        details.push({ issueId: issue.id, fixed: true, message: 'Fixed successfully' });
      } catch (error) {
        failed++;
        details.push({ issueId: issue.id, fixed: false, message: error.message });
      }
    }
  ...
  getWCAGGuidelines(): WCAGGuideline[] {
    return [
      { id: '1.1.1', name: 'Non-text Content', level: 'A', description: 'All non-text content has a text alternative' },
      { id: '1.3.1', name: 'Info and Relationships', level: 'A', description: 'Information and relationships are programmatically determined' },
      { id: '1.3.2', name: 'Meaningful Sequence', level: 'A', description: 'Reading order is logical and meaningful' },
      ...
      { id: '1.4.4', name: 'Resize Text', level: 'AA', description: 'Text can be resized up to 200% without loss' },
      { id: '1.4.6', name: 'Contrast (Enhanced)', level: 'AAA', description: 'Text has a contrast ratio of at least 7:1' },
      { id: '2.4.2', name: 'Page Titled', level: 'A', description: 'Pages have titles that describe topic or purpose' },
      { id: '2.4.6', name: 'Headings and Labels', level: 'AA', description: 'Headings and labels describe topic or purpose' },
      { id: '3.1.1', name: 'Language of Page', level: 'A', description: 'Default language is programmatically determined' },
    ];
  }

  // Private helper methods
  private checkSlideStructure(slide: any): AccessibilityIssue[] {
    const issues: AccessibilityIssue[] = [];
    const blocks = slide.blocks || [];

    // Check for heading hierarchy
    const headings = blocks.filter((b: any) => 
    ...
    if (headings.length === 0) {
      issues.push({
        ...
        description: 'Slide does not have a heading, which helps with navigation',
        wcagCriteria: '2.4.6',
    ...
    // Check reading order
    const orderedBlocks = blocks.sort((a: any, b: any) => a.order - b.order);
  ...
  private checkBlock(block: any, themeColors: any, slideId: string): AccessibilityIssue[] {
    const issues: AccessibilityIssue[] = [];
    const content = block.content || {};

    // Check images for alt text
    if (block.blockType === 'IMAGE') {
      const altText = content.alt || content.altText || '';
    ...
    if (textBlockTypes.includes(block.blockType)) {
      ...
      const textColor = style.color || themeColors.text || '#000000';
      const bgColor = style.backgroundColor || themeColors.background || '#ffffff';
    ...
    if (block.blockType === 'CHART' || block.blockType === 'STATS_GRID') {
      const hasLabels = content.showDataLabels !== false;
  ...
  private calculateScore(issues: AccessibilityIssue[]): { score: number; grade: 'A' | 'B' | 'C' | 'D' | 'F' } {
    let score = 100;
  ...
  private adjustColorForContrast(
    ...
    for (let i = 0; i < 100; i++) {
      const adjusted = shouldLighten
        ? { r: Math.min(255, rgb.r + 5), g: Math.min(255, rgb.g + 5), b: Math.min(255, rgb.b + 5) }
        : { r: Math.max(0, rgb.r - 5), g: Math.max(0, rgb.g - 5), b: Math.max(0, rgb.b - 5) };
  ...
  private async applyAutoFix(issue: AccessibilityIssue): Promise<void> {
    ...
    switch (issue.category) {
      case 'alt-text':
        const altText = await this.generateAltText(issue.autoFixData.imageUrl);
        await this.prisma.block.update({
          where: { id: issue.blockId },
          data: {
            content: {
              ...(await this.prisma.block.findUnique({ where: { id: issue.blockId } }))?.content as any,
              alt: altText,
            },
          },
        });
      ...
      case 'font-size':
        await this.prisma.block.update({
          ...
          data: {
            style: {
              ...(await this.prisma.block.findUnique({ where: { id: issue.blockId } }))?.style as any,
              fontSize: issue.autoFixData.newFontSize,
            },
          },
        });
        break;
      ...
      case 'contrast':
        const suggestion = await this.suggestAccessibleColors(
          issue.autoFixData.foreground,
          issue.autoFixData.background,
        );
        await this.prisma.block.update({
          where: { id: issue.blockId },
          data: {
