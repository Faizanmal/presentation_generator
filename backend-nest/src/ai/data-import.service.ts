    ...
    private recommendCharts(
        ...
        if (dateColumns.length > 0 && numericColumns.length > 0) {
            recommendations.push({
                ...
                reason: 'Time-series data is best visualized with line charts to show trends over time',
            });
        }
        ...
        if (categoricalColumns.length > 0 && numericColumns.length > 0) {
            recommendations.push({
                ...
                reason: 'Bar charts effectively compare numerical values across categories',
            });
        }
        ...
        if (numericColumns.length >= 2) {
            recommendations.push({
                ...
                reason: 'Scatter plots reveal correlations between two numerical variables',
            });
        }
...
5. Concludes with recommendations`;
            ...
            if (charts.length > 0) {
                let chartIndex = 0;
                for (let i = 1; i < presentation.sections.length && chartIndex < charts.length; i++) {
        ...
        if (columns.length >= 2) {
            ...
            dataColumns.forEach((colName, idx) => {
                ...
                datasets.push({
                    label: colName,
                    data,
                    backgroundColor: chartType === ChartTypeEnum.PIE || chartType === ChartTypeEnum.DOUGHNUT
                        ? colors
                        : colors[idx % colors.length],
                    borderColor: chartType === ChartTypeEnum.LINE || chartType === ChartTypeEnum.SCATTER
                        ? colors[idx % colors.length]
                        : undefined,
                });
            });
