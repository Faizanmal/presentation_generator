    ...
    try {
      const response = await this.openai.chat.completions.create({
        ...
        messages: [
          {
            role: 'system',
            content: 'You are a presentation design expert. Provide actionable suggestions to improve slide content and design.',
          },
...
Format as JSON:
{
  ...
  async suggestNextSlide(
    ...
    const slideSummaries = currentSlides.map((s, i) => 
      `Slide ${i + 1}: ${this.extractText(s).substring(0, 100)}`
    ).join('\n');
...
Respond with JSON:
{
  ...
  async rewriteContent(
    content: string,
    style: 'professional' | 'casual' | 'concise' | 'detailed' | 'persuasive' | 'educational'
  ): Promise<{ rewritten: string; changes: string[] }> {
    const styleGuides: Record<string, string> = {
      professional: 'Use formal language, avoid contractions, maintain objectivity',
...
JSON:
{
  ...
  private extractText(content: any): string {
    ...
    if (typeof content === 'object' && content !== null) {
      ...
      return Object.values(content).map(v => this.extractText(v)).join(' ');
    }
    return '';
  }
  ...
  private detectSlideType(content: any): string {
    ...
    if (text.includes('thank you') || text.includes('questions')) return 'closing';
    if (text.includes('introduction') || text.includes('welcome')) return 'introduction';
    if (text.includes('comparison') || text.includes(' vs ')) return 'comparison';
    if (text.includes('timeline') || text.includes('roadmap')) return 'timeline';
    if (text.includes('summary') || text.includes('conclusion')) return 'summary';
