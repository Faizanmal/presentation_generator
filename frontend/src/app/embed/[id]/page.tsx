...
export default function PresentPage() {
  ...
  const startSlideParam = searchParams.get('start');
  const showControlsParam = searchParams.get('controls') !== '0';
  const showNavParam = searchParams.get('nav') !== '0';

  const [currentSlideIndex, setCurrentSlideIndex] = useState(startSlideParam ? parseInt(startSlideParam) - 1 : 0);
  const [isFullscreen, setIsFullscreen] = useState(false);
  const [showControls, setShowControls] = useState(showControlsParam);
